...
func _set_window_layout(configuration) -> void:
	_remove_sidebars()
	_current_sidebar_location = configuration.get_value("Components", "sidebar_location", SidebarLocation.LEFT)
	_add_sidebars()


func _get_window_layout(configuration) -> void:
	configuration.set_value("Components", "sidebar_location", _current_sidebar_location)
...
func _on_selection_changed() -> void:
	...
	if selection.size() == 1:
		...
		if selected_node is Component_holder \
		or selected_node is component:
